...
int tcp_close(tcpsock_t **socket) {
    ...
    // ?? N?u con tr? socket NULL ho?c dã b? gi?i phóng t? tru?c ? tr? v? l?i
    if (socket == NULL || *socket == NULL) return TCP_SOCKET_ERROR;

    // ?? Ki?m tra MAGIC_COOKIE d? d?m b?o chua b? free tru?c dó
    if ((*socket)->cookie != MAGIC_COOKIE) {
        // dã b? free r?i ho?c không ph?i socket h?p l?
        return TCP_SOCKET_ERROR;
    }

    // N?u socket h?p l? ? dóng k?t n?i và gi?i phóng
